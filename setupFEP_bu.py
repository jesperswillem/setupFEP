...
class Run(object):
    ...
    ligands
    """
    def __init__(self, lig1, lig2, offset, FF, system, cluster, sphereradius, *args, **kwargs):
        """
        ...
        with open(self.lig1 + '.pdb') as infile,                                            \
            ...
            for line in infile:
                if line.split()[0] == 'ATOM':
                    ...
                    # Temporary fix NaN error of overlapping heavy atom in Q
                    atnr += 1
                    atom1 = line.split()
    ...
    def write_MD(self, lambdas, writedir, lig_size1, lig_size2):
        ...
        lambda_1 = lambda_1[::-1]
        lambda_2 = lambda_2[1:]
        replacements['ATOM_START_LIG1'] =   '{:<6}'.format(self.offset + 1)
        replacements['ATOM_END_LIG1']   =   '{:<7}'.format(self.offset + lig_size1)           
        replacements['ATOM_START_LIG2'] =   '{:<6}'.format(self.offset + lig_size1 + 1)
        replacements['ATOM_END_LIG2']   =   '{:<7}'.format(self.offset + lig_size1 + lig_size2)
        replacements['SPHERE']          =   self.sphereradius
        replacements['ATOM_END']        =   '{:<6}'.format(self.offset + lig_total)        
        
        if self.system == 'water' or 'vacuum':
            replacements['WATER_RESTRAINT'] = '{:<7}{:<7} 2.0 0 0'.format(self.offset + 1, 
                                                                          self.offset + lig_size1 + 
                                                                          lig_size2
                                                                         )
    ...
    def write_qprep(self, writedir):
        replacements = {}
        center = f.COG(writedir + '/' + self.lig1 + '_' + self.lig2 + '.pdb')
...
if __name__ == "__main__":
    ...
    parser.add_argument('-o', '--offset',
                        ...
                        required = False,
                        default = '0',
                        help = "offset of the ligand pdb in the inputfile (atommr -1)")
    
    parser.add_argument('-s', '--system',
                        dest = "system",
                        required = True,
                        choices = ['water', 'protein', 'vacuum'],
                        help = "this option is needed to apply the correct restraints")
